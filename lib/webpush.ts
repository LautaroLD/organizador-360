...
export async function subscribeToPush(
  ...
  try {
    ...
    if (
      ...
      console.warn(
        '⚠️ Found invalid/expired subscription endpoint. Performing full Service Worker reset...',
      );
    ...
    if (!subscription) {
      try {
        ...
        console.error(
          '❌ Failed during subscription creation:',
          subscribeError,
        );
    ...
    if (
      ...
      throw new Error(
        'Browser returned invalid subscription even after reset. Please clear site data.',
      );
    }
    ...
    if (
      ...
      console.warn(
        '🔧 Corrupted subscription detected (Edge-specific issue). Attempting repair...',
      );
      if (userId) {
        sendPushLog(
          userId,
          'warn',
          'Corrupted subscription detected, attempting repair',
          {
            error: errorMessage,
          },
        ).catch(() => {});
...
/**
 * Clear browser push subscription from Service Worker
 */
export async function clearLocalPushSubscription(): Promise<boolean> {
  try {
    ...
    console.error('Failed to clear local push subscription:', error);
    return false;
  }
}
