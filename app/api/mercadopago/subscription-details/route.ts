...
export async function GET() {
  try {
    ...
    try {
      ...
      const isExpired =
        mpSubscription.status === 'cancelled' || (!!endDate && endDate < now);
      ...
      const mpDetails = {
        ...
        pendingChargeQuantity:
          mpSubscription.summarized?.pending_charge_quantity || 0,
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const subscriptionPlanId = (mpSubscription as any).preapproval_plan_id as
        | string
        | undefined;
      const internalPlanId = mapMercadoPagoPlanIdToTier(
        subscriptionPlanId,
        true,
      );
